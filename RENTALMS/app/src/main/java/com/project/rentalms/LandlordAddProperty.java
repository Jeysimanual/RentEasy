...
public class LandlordAddProperty extends AppCompatActivity {
    ...
    private void addPropertyToFirestore() {
        ...
        if (uploadCounter[0] == 2) {
            // Create a new document in the "properties" collection, with automatic ID generation
            DocumentReference landlordDocRef = db.collection("Landlords").document(landlordId)
                    .collection("properties").document(); // Document ID will be automatically generated

            // Set the property data, including the auto-generated document ID as the property ID
            propertyData.put("propertyId", landlordDocRef.getId()); // Set the propertyId to Firestore's document ID
